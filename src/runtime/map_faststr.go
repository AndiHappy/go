...
func mapassign_faststr(t *maptype, h *hmap, s string) unsafe.Pointer {
	...
	//h.flags 是 hamp 中的成员变量，应该是表示 hmap 状态的一个参数，hashWriting=4，标识a goroutine is writing to the map
	if h.flags&hashWriting != 0 {
		fatal("concurrent map writes")
	}
	//s 是 key 值，这里是构建stringStructOf结构体，为什么设置了一个结构体？
	key := stringStructOf(&s)
	//获取 hash 值
	hash := t.Hasher(noescape(unsafe.Pointer(&s)), uintptr(h.hash0))//15320803351821834856

	// Set hashWriting after calling t.hasher for consistency with mapassign.
	h.flags ^= hashWriting //此时的 h.flags=4,被设置为正在写

	if h.buckets == nil {
		//如果 bucket 为nil,则需要新建类型 t 的 bucket 了
		//newobject，其实就是分配具体的内存空间：mallocgc(typ.Size_, typ, true)
...
again:
	...
	tb:=uintptr(t.BucketSize)
	v1 := bucket*tb
	b := (*bmap)(add(h.buckets,v1))
	top := tophash(hash)
